...
Constructor and mutator methods (such as C<new> and C<set>) 
will C<die> if there is an attempt to set the datetime 
to an invalid value.

Invalid values are detected by setting the appropriate
fields of a C<base> datetime object. See the C<set_base> method.

Accessor methods (such as C<day>) will return either a
value or C<undef>. Accessors will never C<die>.
...
C<new> also accepts a C<base> parameter. See the C<set_base> method.
...
It creates a C<DateTime::Incomplete> object with all date fields defined.
Time fields (hour/minute/...) must be explicitly defined.
...
The default value for "base" is C<undef>, which
means no validation is made on input.
